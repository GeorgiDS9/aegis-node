...
test('RED_TEAM_REPORT terminal header activates with violet styling after probe', async ({ page }) => {
  await page.route('/api/red-team/run', (route) =>
    route.fulfill({
      status:      200,
      contentType: 'text/plain; charset=utf-8',
...
test('probe shows error message when route is unreachable', async ({ page }) => {
  ...
  await expect(
    page.getByText(/Red Team Engine offline/i),
