...
import type { VaultSearchResult, ScanAlert, FirewallStatus, HardwareMetrics, DefenseLogEntry } from '@/types/aegis'
...
describe('mapVaultToLogEntries', () => {
  ...
  it('maps outcome to correct entry type', () => {
    expect(mapVaultToLogEntries([makeVaultResult({ outcome: 'enforced' })])[0].type).toBe('success')
    expect(mapVaultToLogEntries([makeVaultResult({ outcome: 'suspended' })])[0].type).toBe('warning')
...
describe('buildScanContext', () => {
  ...
  const alerts: ScanAlert[] = [
    { id: 'a1', file: '/watch/f1', type: 'critical', message: 'Critical file drift', timestamp: '' },
