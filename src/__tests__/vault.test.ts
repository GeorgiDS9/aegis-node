...
const mockTable = {
  ...
  vectorSearch: vi.fn().mockReturnValue({ limit: vi.fn().mockReturnValue({ toArray: mockVsToArray }) }),
}
...
describe('logRemediation', () => {
  ...
  it('includes a vector array of the correct dimension (4096)', async () => {
    await logRemediation({
      id: 'test', cve_id: 'test', target: 'test', action: 'test',
      risk: 'MEDIUM', outcome: 'success', source: 'EDGE',
  ...
  it('falls back to zero vector when Ollama is offline', async () => {
    await logRemediation({
      id: 'test', cve_id: 'test', target: 'test', action: 'test',
      risk: 'LOW', outcome: 'success', source: 'EDGE',
      timestamp: new Date().toISOString(),
    })
...
describe('getDefenseLogs', () => {
  ...
  it('maps all VaultSearchResult fields correctly', async () => {
    mockQueryToArray.mockResolvedValue([makeRawRow({
      id: 'test-id', cve_id: 'CVE-001', target: 'my-target',
      action: 'my-action', risk: 'CRITICAL', outcome: 'enforced',
      source: 'CLOUD', timestamp: '2024-06-01T00:00:00Z',
    })])

    const [entry] = await getDefenseLogs()
    expect(entry).toMatchObject({
      id: 'test-id', cve_id: 'CVE-001', target: 'my-target',
      action: 'my-action', risk: 'CRITICAL', outcome: 'enforced', source: 'CLOUD',
    })
  })
...
describe('searchRemediations', () => {
  ...
  it('converts _distance to a similarity score (1 - distance)', async () => {
    mockVsToArray.mockResolvedValue([
      { ...makeRawRow({ id: 'result-1' }), _distance: 0.3 },
    ])
