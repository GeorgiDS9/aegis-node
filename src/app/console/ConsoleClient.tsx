    ...
    <>
      <main className="min-h-screen bg-[#020617] text-slate-100 selection:bg-violet-500/30">
              ...
              <div className="flex flex-col items-start leading-none gap-1.5">
                ...
                <span className="text-[7px] font-black text-violet-400/60 uppercase tracking-widest pointer-events-none">
                  {authorizedIds.size > 0
                    ? `${authorizedIds.size} Command${authorizedIds.size > 1 ? "s" : ""} Authorized`
        ...
        <div className="mx-auto max-w-[1400px] px-6 pb-32 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <AdaptiveShield activeRules={activeWafRules} onChange={setActiveWafRules} />
            <RedTeamPanel />
          </div>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/20 p-6 flex flex-col justify-between">
      <div className="flex items-center justify-between mb-4">
        ...
        <span
          className={`text-xs font-medium uppercase px-3 py-1 rounded-full border tracking-wide leading-none flex items-center justify-center ${STATUS_BADGE[status]}`}
        >
          {status}
        </span>
      </div>
