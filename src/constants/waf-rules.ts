...
export const WAF_PATTERNS: Record<string, RegExp> = {
  "WAF-SQLi":
    /('--|%27--|;\s*--|\/\*.*\*\/|\bor\b\s+\w+\s*=\s*\w+|\bunion\b\s+\bselect\b|\bdrop\s+table\b|\binsert\s+into\b|\bdelete\s+from\b|xp_\w+)/i,
  "WAF-XSS":
    /(<\s*script[\s>]|javascript\s*:|on\w+\s*=\s*["']|<\s*iframe[\s>]|document\.cookie|eval\s*\()/i,
