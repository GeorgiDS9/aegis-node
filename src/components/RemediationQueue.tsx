        ...
        <div className="space-y-3 flex-1 pr-2 overflow-y-auto custom-scrollbar">
          {activeAlerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-4 opacity-40">
              ...
              <SystemLabel type="empty-header">No active threats</SystemLabel>
            </div>
          ) : (
            activeAlerts.map((alert) => (
              <div key={alert.id} className="mb-3 last:mb-0">
                <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800/80 flex justify-between items-center group transition-all">
                  <div
                    className="min-w-0 pr-4 space-y-1.5"
                    onClick={() => plans[alert.id] && toggleExpand(alert.id)}
                  >
                ...
                {plans[alert.id] !== undefined && (
                  <div
                    className={`flex-1 mt-4 rounded-lg border ${plans[alert.id] ? "border-violet-500/30 shadow-[0_0_25px_-12px_rgba(139,92,246,0.2)]" : "border-slate-800/60"} bg-[#0c1222] px-5 pb-5 shadow-lg max-h-[500px] overflow-y-auto custom-scrollbar relative`}
                  >
                    <div className="flex items-center justify-between mb-4 pb-3 border-b border-violet-500/10 sticky top-0 bg-[#0c1222] z-10 pt-4">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`h-2 w-2 rounded-full bg-violet-400 ${streamingIds.has(alert.id) ? "animate-pulse" : ""}`}
                        />
              ...
              {!vanguardFeed.connected ? (
                <div className="flex flex-col items-center justify-center py-16 gap-5">
                  ...
                  <div className="text-center">
                    <SystemLabel type="empty-header">Node Disconnected</SystemLabel>
                    {vanguardFeed.error && (
                ...
                vanguardFeed.alerts.map((alert) => {
                  ...
                  return (
                    <div
                      key={alert.id}
                      className="rounded-xl border border-slate-800 bg-slate-950/40 p-5 group/item transition-all"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1 space-y-2">
                          ...
                          <p className="text-[14px] font-bold text-slate-300 leading-snug">
                            {alert.message}
                          </p>
                        </div>
                        {cmd && (
                          <AegisButton
                            ...
                            className={
                              isAuthorized
                                ? "!bg-emerald-600/20 !border-emerald-500/30 !text-emerald-400 shadow-[0_0_15px_-5px_rgba(16,185,129,0.3)]"
                                : ""
                            }
                          />
                        )}
                      ...
                      {cmd && (
                        <div className="mt-4 rounded-lg bg-slate-950 px-4 py-2.5 border border-slate-800 shadow-inner group/code">
                          <code className="text-[11px] font-mono text-slate-300 break-all leading-relaxed">
                            <span className="text-slate-700 mr-2 group-hover/code:text-slate-500 transition-colors">
                              $
                            </span>
                            {cmd.command}
                          </code>
