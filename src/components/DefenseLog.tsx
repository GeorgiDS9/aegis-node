    ...
    await streamQuery(
      ...
      () => {
        addEntry({
          type: "ai",
          source: "EDGE",
          message: liveRef.current || "Threat scan complete.",
        });
    ...
    <AegisCard>
      ...
      <div className="space-y-6 relative h-[250px] overflow-y-auto custom-scrollbar pr-2 pt-4">
        ...
        {entries.length === 0 && !isScanning && (
          <div className="relative pl-9">
            <SystemLabel>No vault activity yet — run a scan or deploy a remediation.</SystemLabel>
          </div>
        )}
        ...
        {entries.map((entry) => (
          <div key={entry.id} className="relative pl-9 group">
            <div
              className={`absolute left-[-2px] top-1 h-5 w-5 rounded-full border flex items-center justify-center ${
                entry.type === "ai"
                  ? "border-violet-500/50 bg-[#020617]"
                  : "border-slate-700 bg-[#020617]"
              }`}
            >
              <div
                className={`h-1.5 w-1.5 rounded-full ${entry.type === "ai" ? "bg-violet-400" : "bg-violet-500"}`}
              />
            </div>
            <div className="flex items-center gap-2 mb-0.5">
              <p
                className={`text-[11px] font-black tracking-widest uppercase truncate ${
            ...
            <p className="text-[9px] font-bold text-slate-600 uppercase">{entry.timestamp}</p>
          </div>
        ))}
      </div>
    </AegisCard>
