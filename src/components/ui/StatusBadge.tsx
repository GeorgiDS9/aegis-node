...
export function StatusBadge({
  ...
  const styles = {
    violet:
      "text-violet-400 border-violet-500/30 bg-violet-500/10 shadow-[0_0_10px_rgba(139,92,246,0.1)]",
    ...
    amber:
      "text-amber-400 border-amber-500/30 bg-amber-500/10 shadow-[0_0_10px_rgba(245,158,11,0.1)]",
  ...
  return (
    <span
      className={`font-black uppercase rounded border tracking-widest flex items-center gap-1.5 ${styles[currentType]} ${sizes[size]}`}
    >
      {pulse && (
        <span
          className={`h-1 w-1 rounded-full animate-pulse ${currentType === "emerald" ? "bg-emerald-400" : "bg-current"}`}
        />
      )}
