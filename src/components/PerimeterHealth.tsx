  ...
  return (
    <AegisCard>
      <CardHeader
        ...
        icon={Lock}
        rightElement={<StatusBadge label="Read-Only Auditor" type="default" size="md" />}
      />

      <div className="flex items-center gap-5 mt-2">
        <div
          className={`flex h-14 w-14 items-center justify-center rounded-2xl border flex-shrink-0 transition-all ${
            isError
          ...
          <div className="flex items-center gap-3">
            <p
              className={`text-[12px] font-black uppercase tracking-widest ${
                isError
                  ? "text-blue-400/80 shadow-[0_0_10px_rgba(59,130,246,0.2)]"
                  : status.enabled
                    ? "text-emerald-400/80"
                    : "text-red-400/80"
              }`}
            >
              {isError
                ...
                : status.enabled
                  ? "PF Firewall Active"
                  : "PF Firewall Inactive"}
            </p>
            {isError && <StatusBadge label="Restricted" type="blue" size="sm" />}
            {!isError && (
              <StatusBadge
